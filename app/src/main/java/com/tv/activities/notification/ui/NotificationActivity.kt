package com.tv.activities.notification.ui
...
import com.tv.R
import com.tv.activities.notification.adapter.NotificationListAdapter
import com.tv.activities.notification.viewmodel.NotificationViewModel
import com.tv.baseModels.BaseBindingActivity
import com.tv.databinding.ActivityNotificationBinding
import com.tv.fragments.dialog.AlertDialogFragment
import com.tv.utils.Logger
import com.tv.utils.ObjectHelper
import com.tv.utils.TrackerUtil.MoEngageNotificationManager
...
class NotificationActivity : BaseBindingActivity<ActivityNotificationBinding>()  {
    ...
    private var itemClickListener: NotificationListAdapter.OnItemClickListener? =
        object : NotificationListAdapter.OnItemClickListener {
            ...
            override fun onDeleteClicked(inboxMessage: InboxMessage) {
                showAlertDialog(
                    message = getString(com.tv.R.string.delete_notification_single),
                    positiveLabel = getString(com.tv.R.string.ok),
                    positiveAction = {
                        MoEngageNotificationManager.deleteNotification(inboxMessage)
                        notificationAdapter?.deleteItem(inboxMessage)
                    },
                    negativeLabel = getString(com.tv.R.string.cancel),
                )
            }
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (KsPreferenceKeys.getInstance().currentTheme.equals(
            ...
            binding.tvNoData.setTextColor(ContextCompat.getColor(binding.tvNoData.context, com.tv.R.color.series_detail_description_text_color))
        } else {
            binding.tvNoData.setTextColor(ContextCompat.getColor(binding.tvNoData.context, com.tv.R.color.tph_hint_txt_color))
        }
    ...
    private fun setupListener() {
        binding?.toolbar?.clDelete?.setOnClickListener {
            notificationAdapter?.getDataList()?.let { dataList ->
                showAlertDialog(
                    message = getString(com.tv.R.string.delete_notification_all),
                    positiveLabel = getString(com.tv.R.string.ok),
                    positiveAction = {
    ...
    private fun showNoNetwork() {
        ...
        binding?.noConnectionLayout?.findViewById<View>(com.tv.R.id.retry_txt)
    ...
    private fun initRecyclerView(list: List<InboxMessage>) {
        ...
        binding?.rvNotification?.let { rv ->
            ...
            dividerItemDecoration.setDrawable(
                ColorDrawable(ContextCompat.getColor(rv.context, com.tv.R.color.order_history_title_color))
            )
