package com.breadgangtvnetwork.utils.helpers.shimmer;
...
public class ShimmerLayout extends FrameLayout {
    ...
    public ShimmerLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        try {
            ...
            shimmerColor = a.getColor(R.styleable.ShimmerLayout_shimmer_color, getColor());
    ...
    private void createShimmerPaint() {
        ...
        final float shimmerLineWidth = getWidth() /(float) 2 * maskWidth;
    ...
    private int getColor() {
        if (KsPreferenceKeys.getInstance().getCurrentTheme().equalsIgnoreCase(AppConstants.LIGHT_THEME)) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                return getContext().getColor(R.color.app_bg_color);
            } else {
                return getResources().getColor(R.color.app_bg_color);
            }
        } else {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                return getContext().getColor(R.color.app_bg_color);
    ...
    private int calculateMaskWidth() {
        final double shimmerLineBottomWidth = (getWidth() / (double)2 * maskWidth) / Math.cos(Math.toRadians(Math.abs(shimmerAngle)));
